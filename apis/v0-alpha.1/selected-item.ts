...
import type {
	...
	QueringUpdateMany,
	QueringDeleteMany,
} from 'schemas/v0-alpha.1/selected-item';
...
const route: FastifyPluginAsync = async (server, opts) => {
	...
	const api = 'selected-item';
	const { jwt, orm, rbac, totp } = server;
	const model = new SelectedItemModel(orm);
	...
	server.route<{
		...
		handler: async (request, reply) => {
			const user = await request.identify();
			const result = await model.setUser(user).batch(request.body);
			model.unsetUser();
			reply.ok(result);
		},
		schema: {},
