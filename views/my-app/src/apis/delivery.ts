...
import type {
	...
	QueringUpdateMany,
	QueringDeleteMany,
} from '$server/schemas/v0-alpha.1/delivery';
export type {
	Data,
	DataHistory,
	GetQuery,
...
export default class DeliveryClientApi {
	api: ClientApi;
	token: Token;
	...
	public async batch<Query extends Array<BatchQuery>>(data: Query) {
		const response = await this.api
			.request({
				endpoint: 'batch',
				method: 'POST',
	...
	public async searchMany<Query extends SearchManyQuery>(
		data?: ValidateQuery<Query, SearchManyQuery>
	) {
		const response = await this.api
			.request<Query>({
				endpoint: 'searchMany',
	...
	public async createMany<Query extends CreateManyQuery>(
		data: ValidateQuery<Query, CreateManyQuery>
	) {
		const response = await this.api
			.request<Query>({
				endpoint: 'createMany',
	...
	public async updateMany<Query extends UpdateManyQuery>(
		data: ValidateQuery<Query, UpdateManyQuery>
	) {
		const response = await this.api
			.request<Query>({
				endpoint: 'updateMany',
	...
	public async deleteMany<Query extends DeleteManyQuery>(
		data: ValidateQuery<Query, DeleteManyQuery>
	) {
		const response = await this.api
			.request<Query>({
				endpoint: 'deleteMany',
