<script context="module" lang="ts">
	import { MaterialAppMin, Footer, Button, TextField } from 'svelte-materialify/src';
			...
			for (const item of items) {
				...
				if (stores.has(id)) {
					...
					const response = await Map.mbxDirections
						.getDirections({
							profile: 'driving',
							waypoints: [{ coordinates: address.position as any }, { coordinates: position }]
						})
						.send();
					range.push(
						(response.body as Map.Directions).routes.reduce((prev, curr) => prev + curr.distance, 0)
					);
				}
			}
...
<div transition:slide>
	<MaterialAppMin>
		...
		/>
		<Appbar loading={$loading} desktop={$is_desktop} title="Checkout" back_nav />
		<main class="main">
			<form id="order" class="form" on:submit|preventDefault={submit}>
				<h2 class="t-2">Lokasi dan Waktu</h2>
				<section class="section card">
					<h3 class="t-3">Lokasi</h3>
					<Address bind:data={address} menu={addressList} editAll={editAddress} loader={$loading} />
				</section>
				<section class="section card">
