...
<script lang="ts">
  ...
  let transactions: (BuyerClient.Transaction & {
      ...
      <List>
        {#each transactions as transaction}
          <Card>
            <ListItem
              on:click={() => {
                goto("/cleaner/order/" + transaction.id);
              }}
            >
              <span slot="prepend">
                <Icon path={mdiCubeOutline} />
              </span>
              <span>{transaction.created_by.name} ({transaction.status})</span>
              <span slot="subtitle"
                >{new Date(transaction.created_on).toLocaleString()}</span
              >
            </ListItem>
          </Card>
        {/each}
