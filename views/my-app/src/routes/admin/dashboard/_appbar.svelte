...
<script lang="ts">
	...
	export let drawerOpened = true;
	export let account = { image: '', name: '', role: '' };
</script>

<button on:click={() => (drawerOpened = !drawerOpened)} class="btn btn-square btn-ghost">
	{#if drawerOpened}
		<svg
...
<div class="form-control">
	<div class="relative w-full">
		<div class="absolute top-0 left-0 grid place-content-center h-full" style="aspect-ratio: 1;">
			<button type="button" class="btn btn-sm btn-square bg-transparent hover:bg-base-300 border-0">
				<svg
					class="w-6 h-6 text-gray-900 dark:text-gray-50"
					fill="none"
					stroke="currentColor"
					viewBox="0 0 24 24"
					xmlns="http://www.w3.org/2000/svg"
		...
		<input
			...
			type="search"
			placeholder="Search..."
			class="input input-bordered bg-transparent w-full pl-[50px] focus:ring-2 focus:ring-primary-focus focus:ring-offset-2 focus:ring-offset-base-100"
		/>
	</div>
</div>
<div class="flex-grow" />
	...
	{#if account.image}
		<div class="w-10 h-10 m-1">
			<img
				...
				class="aspect-1 object-cover object-center rounded-full"
			/>
		</div>
	{:else}
